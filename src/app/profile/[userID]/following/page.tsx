...
export default function UserFollowingPage() {
  ...
  useEffect(() => {
    const loadUserData = async () => {
      ...
      try {
        // Load user profile
        const profile = await UserService.getUserByFirebaseUid(userID);
        if (!profile) {
  ...
  const handleFollowChange = () => {
    // Reload the following list when follow status changes
    if (userID && typeof userID === 'string') {
      UserService.getFollowing(userID)
        .then(setFollowing)
        .catch(err => {
          console.error('Error reloading following:', err);
        });
    }
  };
        ...
        <div className="max-w-4xl mx-auto p-4">
          ...
          <div className="mb-8">
            <div className="flex items-center mb-4">
              <Link 
                href={`/profile/${userProfile.firebaseUid}`}
                className="mr-4 text-gray-600 hover:text-gray-800 transition-colors"
              >
                ← Back to {userProfile.name}'s Profile
              </Link>
            </div>
            ...
            <p className="text-gray-600 mt-2">
              People {userProfile.name} is following ({following.length})
            </p>
          </div>
          ...
          {following.length === 0 ? (
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center">
              ...
              <p className="text-gray-600 mb-4">
                When {userProfile.name} follows people, they'll appear here.
              </p>
            </div>
          ) : (
