...
import type { Post, TotemLike } from '@/types/modelsV2';

export class TotemServiceV2 {
  /**
   ...
   */
  static async hasUserLiked(postId: string, totemName: string, userId: string): Promise<boolean> {
    const postRef = doc(db, "posts", postId);
  ...
  static async toggleLike(
    ...
    try {
      ...
      const isCurrentlyLiked = await this.hasUserLiked(postId, totemName, userId);
      
      // Toggle the like state
      await this.handleTotemLike(postId, totemName, userId, isCurrentlyLiked);
