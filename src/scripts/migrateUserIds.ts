...
export async function migrateUserIds() {
  ...
  const uidToUsernameMap = new Map<string, string>();
  const usernameToUidMap = new Map<string, string>();
    ...
    if (Array.isArray(postData.answers)) {
      ...
      if (Array.isArray(postData.answerUserIds)) {
        ...
        updates.answerUsernames = postData.answerUserIds.map(
          (uid: string) => uidToUsernameMap.get(uid) || `user_${uid.substring(0, 8)}`
        );
        // Don't remove old fields yet for backward compatibility
...
// Only execute the migration if this file is run directly
